...
from src.measures.validation import (
    minmax_normalize_x,
    validate_histogram,
    validate_parameters,
)
...
class TestValidation(unittest.TestCase):
    ...
    def test_validate_histogram_does_not_normalize_weights_by_default(self):
        """Weights should be preserved unless normalization is explicitly requested."""
    ...
    def test_validate_histogram_can_normalize_weights_explicitly(self):
        """Optional weight normalization should preserve the previous public behavior."""
        x = np.array([0.0, 0.5, 1.0])
        weights = np.array([2.0, 3.0, 5.0])

        x_valid, w_valid = validate_histogram(x, weights, normalize_weights=True)

        np.testing.assert_array_almost_equal(x_valid, x)
        np.testing.assert_array_almost_equal(w_valid, np.array([0.2, 0.3, 0.5]))
