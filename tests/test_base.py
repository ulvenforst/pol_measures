...
from src.measures.base import ParametricPolarizationMeasure, PolarizationMeasure
...
class TestPolarizationMeasure(unittest.TestCase):
    ...
    def test_call_preserves_raw_weights_by_default(self):
        """The public call should not normalize weights unless explicitly requested."""
        raw_weights = np.array([2.0, 2.0, 2.0, 2.0, 2.0])

        result = self.measure(self.x, raw_weights)
        expected = self.measure.compute(self.x, raw_weights)

        self.assertAlmostEqual(result, expected)

    def test_call_can_normalize_weights_explicitly(self):
        """Explicit normalization should still be available through __call__."""
        raw_weights = np.array([2.0, 2.0, 2.0, 2.0, 2.0])

        result = self.measure(self.x, raw_weights, normalize_weights=True)
        expected = self.measure.compute(self.x, raw_weights / np.sum(raw_weights))
