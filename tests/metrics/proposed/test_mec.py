...
class TestMEC(unittest.TestCase):
    ...
    def test_call_uses_raw_weights_by_default(self):
        """The public call should preserve raw masses unless normalization is requested."""
        measure = MEC(alpha=1.0, beta=1.0)
        x = np.array([0.0, 0.583, 1.0])
        weights = np.array([0.714, 0.05, 0.714])

        raw_result = measure(x, weights)
        normalized_result = measure(x, weights, normalize_weights=True)

        self.assertAlmostEqual(raw_result, measure.compute(x, weights))
        self.assertAlmostEqual(normalized_result * np.sum(weights), raw_result)
