...
from .thresholds import CATEGORY_LABELS, THRESHOLDS
...
class PolarizationMeasure(ABC):
    ...
    def _has_parameter_sets(self) -> bool:
        ...
        return (
            self.measure_id in THRESHOLDS and "_params" in THRESHOLDS[self.measure_id]
        )
    ...
    def find_matching_parameter_set(self) -> Optional[str]:
        ...
        try:
            if hasattr(self, "get_parameters") and callable(
                getattr(self, "get_parameters")
            ):
            ...
            for param_set_key, default_params in param_sets.items():
                ...
                for param, default_value in default_params.items():
                    ...
                    if isinstance(default_value, (int, float)) and isinstance(
                        current_params[param], (int, float)
                    ):
                        if not math.isclose(
                            default_value,
    ...
    def __call__(
        ...
        *,
        ...
        Args:
            ...
            normalize_weights: Whether to normalize weights to sum to 1 before computing
            normalize_positions: Whether to rescale x to [0, 1] before computing
        ...
        if isinstance(labels, int) and labels >= 2:
            ...
            try:
                category = self._classify_value(
                    self._cached_result,
                    num_categories=labels,
                    method=method,
                    param_set=param_set,
                )
        ...
        raise ValueError(
            f"Invalid value for 'labels': {labels}. Must be None, a positive integer, or 'all'."
        )

    def _get_thresholds(
        self, num_categories: int, method: str = "kmeans", param_set: str = "default"
    ) -> List[float]:
        """
        ...
        if self._has_parameter_sets():
            if param_set not in thresholds_data:
                raise ValueError(
                    f"Parameter set '{param_set}' not found for measure: {self.measure_id}"
                )
        ...
        if method not in thresholds_data:
            raise ValueError(
                f"Method '{method}' not available for measure: {self.measure_id}"
            )

        if num_categories not in thresholds_data[method]:
            raise ValueError(
                f"No {method} thresholds for {num_categories} categories in measure: {self.measure_id}"
            )
    ...
    def _classify_value(
        ...
        labels = CATEGORY_LABELS.get(
            num_categories, [f"category_{i + 1}" for i in range(num_categories)]
        )
    ...
    def _get_all_classifications(
        self, value: float, param_set: str = "default"
    ) -> Dict[str, Any]:
        """
        ...
        for method_name, method_data in thresholds_data.items():
            ...
            for num_cats in method_data:
                try:
                    category = self._classify_value(
                        value, num_cats, method_name, param_set
                    )
